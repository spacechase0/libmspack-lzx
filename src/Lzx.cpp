...
namespace lzx
{
    /*public*/ BitBuffer::BitBuffer(std::istream& stream)
    :   byteStream( stream )
    {
        InitBitStream();
    }

    /*public*/ void BitBuffer::InitBitStream()
    {
        buffer = 0;
        bitsleft = 0;
    }

    /*public*/ void BitBuffer::EnsureBits(sf::Uint8 bits)
    {
        while(bitsleft < bits) {
            int lo = (sf::Uint8)byteStream.get();
            int hi = (sf::Uint8)byteStream.get();
            int amount2shift = sizeof(sf::Uint32)*8 - 16 - bitsleft;
            buffer |= (sf::Uint32)(((hi << 8) | lo) << (sizeof(sf::Uint32)*8 - 16 - bitsleft));
            bitsleft += 16;
        }
    }

    /*public*/ sf::Uint32 BitBuffer::PeekBits(sf::Uint8 bits)
    {
        return (buffer >> ((sizeof(sf::Uint32)*8) - bits));
    }

    /*public*/ void BitBuffer::RemoveBits(sf::Uint8 bits)
    {
        buffer <<= bits;
        bitsleft -= bits;
    }

    /*public*/ sf::Uint32 BitBuffer::ReadBits(sf::Uint8 bits)
    {
        sf::Uint32 ret = 0;
    ...
    /*public*/ sf::Uint32 BitBuffer::GetBuffer()
    {
        return buffer;
    }

    /*public*/ sf::Uint8 BitBuffer::GetBitsLeft()
    {
        return bitsleft;
    }
    //#endregion

    std::vector<sf::Uint32> LzxDecoder::position_base;
    std::vector<sf::Uint8> LzxDecoder::extra_bits;

    /*public*/ LzxDecoder::LzxDecoder (int window)
    {
        sf::Uint32 wndsize = (sf::Uint32)(1 << window);
        int posn_slots;
        ...
        if(extra_bits.size() == 0)
        {
            extra_bits.resize(52);
            for(int i = 0, j = 0; i <= 50; i += 2)
            {
                extra_bits[i] = extra_bits[i+1] = (sf::Uint8)j;
                if ((i != 0) && (j < 17)) j++;
            }
        }
        if(position_base.size()==0)
        {
            position_base.resize(51);
            for(int i = 0, j = 0; i <= 50; i++)
            {
                position_base[i] = (sf::Uint32)j;
                j += 1 << extra_bits[i];
            }
        }
        ...
        m_state.main_elements = (sf::Uint16)(LzxConstants::NUM_CHARS + (posn_slots << 3));
    ...
    /*public*/ int LzxDecoder::Decompress(std::istream& inData, int inLen, std::iostream& outData, int outLen)
    {
        BitBuffer bitbuf = BitBuffer(inData);
        long startpos = inData.tellg();
        long endpos = inData.tellg() + inLen;
        
        std::vector<sf::Uint8>& window = m_state.window;
        
        sf::Uint32 window_posn = m_state.window_posn;
        sf::Uint32 window_size = m_state.window_size;
        sf::Uint32 R0 = m_state.R0;
        sf::Uint32 R1 = m_state.R1;
        sf::Uint32 R2 = m_state.R2;
        sf::Uint32 i, j;
        ...
        //Console.WritezLine("init:" + m_state.block_type + " " + m_state.block_length + " " + inData.tellg() + ":" + bitbuf.bitsleft);

        /* read header if necessary */
        if (m_state.header_read == 0)
        {
            sf::Uint32 intel = bitbuf.ReadBits(1);
            if(intel != 0)
            {
                // read the filesize
                i = bitbuf.ReadBits(16); j = bitbuf.ReadBits(16);
                m_state.intel_filesize = (int)((i << 16) | j);
                //Console.WritezLine("read header: " + m_state.intel_filesize);
            }
            m_state.header_read = 1;
        }
        //Console.WritezLine("header:" + m_state.block_type + " " + m_state.block_length + " " + inData.tellg() + ":" + bitbuf.bitsleft);
        ...
        while (togo > 0)
        {
            ...
            if(m_state.block_remaining == 0)
            {
                ...
                //Console.WritezLine("prebt:" + inData.tellg()+":"+ bitbuf.bitsleft);

                m_state.block_type = (LzxConstants::BLOCKTYPE)bitbuf.ReadBits(3); ;
                //Console.WritezLine("midbit:" + m_state.block_type + " " + inData.tellg() + ":" + bitbuf.bitsleft);
                i = bitbuf.ReadBits(16);
                j = bitbuf.ReadBits(8);
                m_state.block_remaining = m_state.block_length = (sf::Uint32)((i << 8) | j);

                //Console.WritezLine("postbt:" + m_state.block_type + " " + i+" "+j+" "+m_state.block_length + " " + inData.tellg() + ":" + bitbuf.bitsleft);

                switch (m_state.block_type)
                {
                case LzxConstants::BLOCKTYPE::ALIGNED:
                    for(i = 0, j = 0; i < 8; i++) { j = bitbuf.ReadBits(3); m_state.ALIGNED_len[i] = (sf::Uint8)j; }
                ...
                case LzxConstants::BLOCKTYPE::UNCOMPRESSED:
                    ...
                    if(bitbuf.GetBitsLeft() > 16) inData.seekg(-2, std::istream::cur); /* and align the bitstream! */
                    sf::Uint8 hi, mh, ml, lo;
                    lo = (sf::Uint8)inData.get(); ml = (sf::Uint8)inData.get(); mh = (sf::Uint8)inData.get(); hi = (sf::Uint8)inData.get();
                    R0 = (sf::Uint32)(lo | ml << 8 | mh << 16 | hi << 24);
                    lo = (sf::Uint8)inData.get(); ml = (sf::Uint8)inData.get(); mh = (sf::Uint8)inData.get(); hi = (sf::Uint8)inData.get();
                    R1 = (sf::Uint32)(lo | ml << 8 | mh << 16 | hi << 24);
                    lo = (sf::Uint8)inData.get(); ml = (sf::Uint8)inData.get(); mh = (sf::Uint8)inData.get(); hi = (sf::Uint8)inData.get();
                    R2 = (sf::Uint32)(lo | ml << 8 | mh << 16 | hi << 24);
                    break;
            ...
            while((this_run = (int)m_state.block_remaining) > 0 && togo > 0)
            {
                ...
                togo -= this_run;
                m_state.block_remaining -= (sf::Uint32)this_run;
                ...
                case LzxConstants::BLOCKTYPE::VERBATIM:
                    while(this_run > 0)
                    {
                        ...
                        if(main_element < LzxConstants::NUM_CHARS)
                        {
                            ...
                            window[window_posn++] = (sf::Uint8)main_element;
                            this_run--;
                        }
                        else
                        {
                            ...
                            if(match_offset > 2)
                            {
                                ...
                                if(match_offset != 3)
                                {
                                    ...
                                    verbatim_bits = (int)bitbuf.ReadBits((sf::Uint8)extra);
                                ...
                                R2 = R1; R1 = R0; R0 = (sf::Uint32)match_offset;
                            }
                            ...
                            {
                                match_offset = (int)R1;
                                R1 = R0; R0 = (sf::Uint32)match_offset;
                            }
                            else /* match_offset == 2 */
                            {
                                match_offset = (int)R2;
                                R2 = R0; R0 = (sf::Uint32)match_offset;
                            }
                            ...
                            else
                            {
                                ...
                                if(copy_length < match_length)
                                {
                                    ...
                                    window_posn += (sf::Uint32)copy_length;
                                    while(copy_length-- > 0) window[rundest++] = window[runsrc++];
                                    runsrc = 0;
                                }
                            }
                            window_posn += (sf::Uint32)match_length;
                ...
                case LzxConstants::BLOCKTYPE::UNCOMPRESSED:
                    if((inData.tellg() + this_run) > endpos) return -1; //TODO throw proper exception
                    inData.read(reinterpret_cast< char* >( &window[ window_posn ] ), this_run);
                    window_posn += (sf::Uint32)this_run;
                    break;
        ...
        outData.write(reinterpret_cast<char*>(&window[ start_window_pos ] ), outLen);
        ...
        if((m_state.frames_read++ < 32768) && m_state.intel_filesize != 0)
        {
            ...
            else
            {
                ...
                sf::Uint32 curpos = (sf::Uint32)m_state.intel_curpos;
                sf::Uint32 filesize = (sf::Uint32)m_state.intel_filesize;
                sf::Uint32 abs_off, rel_off;
                
                m_state.intel_curpos = (int)curpos + outLen;
                
                outData.seekg( outData.tellp() );
                while(outData.tellg() < dataend)
                {
                    outData.seekp( 1, std::iostream::cur );
                    if(outData.get() != 0xE8) { curpos++; continue; }
                    //abs_off = 
                }
            }
            return -1;
    ...
    /*private*/ int LzxDecoder::MakeDecodeTable(sf::Uint32 nsyms, sf::Uint32 nbits, std::vector<sf::Uint8>& length, std::vector<sf::Uint16>& table)
    {
        sf::Uint16 sym;
        sf::Uint32 leaf;
        sf::Uint8 bit_num = 1;
        sf::Uint32 fill;
        sf::Uint32 pos			= 0; /* the current position in the decode table */
        sf::Uint32 table_mask		= (sf::Uint32)(1 << (int)nbits);
        sf::Uint32 bit_mask		= table_mask >> 1; /* don't do 0 length codes */
        sf::Uint32 next_symbol	= bit_mask;	/* base of allocation for long codes */
        ...
        if(pos != table_mask)
        {
            ...
            for(sym = (sf::Uint16)pos; sym < table_mask; sym++) table[sym] = 0;
                    ...
                    if(length[sym] == bit_num)
                    {
                        ...
                        for(fill = 0; fill < bit_num - nbits; fill++)
                        {
                            ...
                            if(table[leaf] == 0)
                            {
                                ...
                                table[leaf] = (sf::Uint16)(next_symbol++);
                            }
                            /* follow the path and select either left or right for next bit */
                            leaf = (sf::Uint32)(table[leaf] << 1);
    ...
    /*private*/ void LzxDecoder::ReadLengths(std::vector<sf::Uint8>& lens, sf::Uint32 first, sf::Uint32 last, BitBuffer& bitbuf)
    {
        sf::Uint32 x, y;
        int z;
        ...
        {
            y = bitbuf.ReadBits(4);
            //Console.WriteLine("read pt data:" + y);
            m_state.PRETREE_len[x] = (sf::Uint8)y;
        }
    ...
    /*private*/ sf::Uint32 LzxDecoder::ReadHuffSym(std::vector<sf::Uint16>& table, std::vector<sf::Uint8>& lengths, sf::Uint32 nsyms, sf::Uint32 nbits, BitBuffer& bitbuf)
    {
        sf::Uint32 i, j;
        bitbuf.EnsureBits(16);
        if((i = table[bitbuf.PeekBits((sf::Uint8)nbits)]) >= nsyms)
        {
            j = (sf::Uint32)(1 << (int)((sizeof(sf::Uint32)*8) - nbits));
            do
            {
                j >>= 1; i <<= 1; i |= (bitbuf.GetBuffer() & j) != 0 ? (sf::Uint32)1 : 0;
                if(j == 0) return 0; // TODO throw proper exception
            } while((i = table[i]) >= nsyms);
        }
        j = lengths[i];
        bitbuf.RemoveBits((sf::Uint8)j);
